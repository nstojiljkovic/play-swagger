...
class SwaggerParameterMapperSpec extends Specification {
  "mapParam" >> {
    ...
    "map org.joda.time.DateTime to integer with format epoch" >> {
      mapParam(Parameter("fieldWithDateTime", "org.joda.time.DateTime", None, None, None)) === GenSwaggerParameter(
    ...
    "override mapping to map DateTime to string with format date-time" >> {
      "single DateTime" >> {
        ...
        val parameter = mapParam(Parameter("fieldWithDateTimeOverRide", "org.joda.time.DateTime", None, None, None), customMappings = mappings)
      ...
      "sequence of DateTimes" >> {
        ...
        val parameter = mapParam(Parameter("seqWithDateTimeOverRide", "Option[Seq[org.joda.time.DateTime]]", None, None, None), customMappings = mappings).asInstanceOf[GenSwaggerParameter]
    ...
    "add new custom type mapping" >> {
      ...
      val mappings: CustomMappings = List(CustomTypeMapping(
        "java.util.Date",
        specAsParameter = specAsParameter))
      val parameter = mapParam(Parameter("fieldWithDate", "java.util.Date", None, None, None), customMappings = mappings)
    ...
    "map Any to any with example value" >> {
      mapParam(Parameter("fieldWithAny", "Any", None, None, None)) === GenSwaggerParameter(
        name = "fieldWithAny",
        `type` = Option("any"),
    ...
    "map java enum to enum constants" >> {
      mapParam(Parameter("javaEnum", "com.github.nstojiljkovic.playSwagger.SampleJavaEnum", None, None, None)) === GenSwaggerParameter(
        name = "javaEnum",
        `type` = Option("string"),
    ...
    "map scala enum to enum constants" >> {
      mapParam(Parameter("scalaEnum", "com.github.nstojiljkovic.playSwagger.SampleScalaEnum.Value", None, None, None)) === GenSwaggerParameter(
        name = "scalaEnum",
        `type` = Option("string"),
    ...
    "map Option[Seq[T]] to item type" >> {
      mapParam(Parameter("aField", "Option[Seq[String]]", None, None, None)) === GenSwaggerParameter(
        name = "aField",
        required = false,
        `type` = Some("array"),
    ...
    "map String to string without override interference" >> {
      ...
      val mappings: CustomMappings = List(CustomTypeMapping(
        ...
        "java.time.Duration",
        specAsParameter = specAsParameter))
      val parameter = mapParam(Parameter("strField", "String", None, None, None), customMappings = mappings)
    ...
    "map default value to content without quotes when provided with string without quotes" >> {
      mapParam(Parameter("strField", "String", None, Some("defaultValue"), None)) === GenSwaggerParameter(
        name = "strField",
        `type` = Option("string"),
        required = false,
